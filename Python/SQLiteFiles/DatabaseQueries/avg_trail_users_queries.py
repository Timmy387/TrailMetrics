...
def fill_results(results, full_list, divisor=1):
    res_i = 0
    new_list = []
    for i, num in enumerate(full_list):
        ...
        if num == res_num:
            count = res_count
            res_i += 1
        new_list.append((num, count / divisor))
    return new_list
...
def per_month_each_year(conn: Connection,
    ...
    for i in range(len(new_result)):
        month, count = new_result[i]
        if count != 0:
            new_result[i] = (month, count / num_years)
        else:
            new_result[i] = (month, 0)
    return new_result
...
def per_week_each_year(conn: Connection,
        ...
        start_year = int(start_date.split("-")[0])
        end_year = int(end_date.split("-")[0])
        divisor = end_year - start_year + 1
        new_result = fill_results(result, weeks, divisor)
        return new_result
...
def per_day_of_year_each_year(conn: Connection,
    ...
    divisor = end_month - start_month + 1
    new_result = fill_results(result, days, divisor)
...
def per_day_of_week_each_year(conn: Connection,
    ...
    num_years = end_year - start_year + 1
    for i in range(len(new_result)):
        day, count = new_result[i]
        if count != 0:
            new_result[i] = (day, count / num_years)
        else:
            new_result[i] = (day, 0)
    return new_result
...
def per_time_of_day_each_year(conn: Connection,
    ...
    end_year = int(end_date.split("-")[0])
    num_years = end_year - start_year + 1
    new_result = fill_results(result, times, num_years)
...
def per_day_of_year_each_month(conn: Connection,
        ...
        start_year = int(start_date.split("-")[0])
        end_year = int(end_date.split("-")[0])
        num_months = (end_month - start_month + 1) + (12 * (end_year - start_year))
        new_result = fill_results(result, days, num_months)
        return new_result
