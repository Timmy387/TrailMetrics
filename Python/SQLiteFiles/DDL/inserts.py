...
def delete_file(conn: Connection, file_name: str) -> bool:
    """
    deletes a file listing from the files table
    :param conn: connection to the trail_metrics DB
    :param file_name: name of file being removed
    :return: true if successful
    """
    check_for_file = """
        SELECT count(*) 
        ...
        WHERE file_name = ?;
        """
    if conn.execute(check_for_file, (file_name,)).fetchone()[0] == 0:
        return False
    delete = """
        DELETE FROM files WHERE file_name = ?;
        """
    conn.execute(delete, (file_name,))
    conn.commit()
    return True